...
class TestDeliveryService:
    ...
    async def test_create_order_success(
        self,
        delivery_service: DeliveryOrderService,
        ...
        expected_order = OrderDTO(
            ...
            total=Decimal("31.97"),  # (2 * 12.99) + 5.99 + 5.00 delivery fee
        ...
        # When
        result = await delivery_service.create_order(order_input, user)
    ...
    async def test_create_order_invalid_service_type(
        ...
        # Given
        order_input = OrderInputDTO(
            service_type=ServiceType.DINE_IN,  # Invalid for delivery service
            items=[
                OrderItemInputDTO(
                    type="pizza",
                    product_name="Margherita",
        ...
        # When/Then
        with pytest.raises(
            ValueError, match="Invalid service type for delivery service"
        ):
            await delivery_service.create_order(order_input, user)

    @pytest.mark.asyncio
    async def test_create_order_nonexistent_pizza(
        self,
        delivery_service: DeliveryOrderService,
        mock_pizza_dao: AsyncMock,
        user: UserReadDTO,
    ...
    async def test_check_status_success(
        ...
        # When
        result = await delivery_service.check_status(order_id, user)
    ...
    @pytest.mark.asyncio
    async def test_check_status_nonexistent_order(
        self,
        delivery_service: DeliveryOrderService,
        mock_order_dao: AsyncMock,
        mock_uow: AsyncMock,
        ...
        with pytest.raises(NotFoundError, match=f"Order {order_id} not found"):
            await delivery_service.check_status(order_id, user)
    ...
    async def test_update_order_success(
        ...
        expected_updated_order = OrderDTO(
            ...
            total=Decimal("30.98"),  # (2 * 12.99) + 5.00 delivery fee
            customer_email=user.email,
            notes="Updated order",
            created_at=now,
            updated_at=now,
        ...
        # When
        result = await delivery_service.update_order(
    ...
    async def test_update_order_not_found(
        ...
        # When/Then
        with pytest.raises(NotFoundError, match=f"Order {order_id} not found"):
            await delivery_service.update_order(order_id, update_input, user)
    ...
    async def test_cancel_order_success(
        ...
        # When
        result = await delivery_service.cancel_order(
    ...
    async def test_cancel_order_unauthorized(
        ...
        with pytest.raises(
            ...
            await delivery_service.cancel_order(order_id, user)
    ...
    async def test_update_order_invalid_item_type(
        ...
        with pytest.raises(
            ...
            await delivery_service.update_order(order_id, order_input, user)
