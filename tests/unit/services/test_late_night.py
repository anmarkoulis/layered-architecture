...
class TestLateNightService:
    @pytest.mark.asyncio
    @freeze_time("2024-03-20 23:00:00")  # 11 PM
    async def test_create_order_success(
        self,
        late_night_service: LateNightOrderService,
        ...
        # When
        result = await late_night_service.create_order(order_input, user)
    ...
    async def test_create_order_invalid_service_type(
        ...
        # Given
        order_input = OrderInputDTO(
            service_type=ServiceType.DELIVERY,  # Invalid for late night service
            items=[
                OrderItemInputDTO(
                    type="pizza",
                    product_name="Margherita",
        ...
        # When/Then
        with pytest.raises(
            ValueError, match="Invalid service type for late night service"
        ):
            await late_night_service.create_order(order_input, user)
    ...
    async def test_check_status_success(
        ...
        # When
        result = await late_night_service.check_status(order_id, user)
    ...
    async def test_check_status_order_not_found(
        ...
        with pytest.raises(NotFoundError, match=f"Order {order_id} not found"):
            await late_night_service.check_status(order_id, user)
    ...
    async def test_check_status_unauthorized(
        ...
        # When/Then
        with pytest.raises(
            ValueError, match="Unauthorized to check this order"
        ):
            await late_night_service.check_status(order_id, user)
    ...
    async def test_update_order_success(
        ...
        # When
        result = await late_night_service.update_order(
    ...
    async def test_update_order_not_found(
        ...
        # When/Then
        with pytest.raises(NotFoundError, match=f"Order {order_id} not found"):
            await late_night_service.update_order(order_id, update_input, user)
    ...
    async def test_update_order_invalid_service_type(
        ...
        # When/Then
        with pytest.raises(
            ValueError, match="Cannot change service type to non-late-night"
        ):
            await late_night_service.update_order(order_id, update_input, user)
    ...
    async def test_cancel_order_success(
        ...
        # When
        result = await late_night_service.cancel_order(
    ...
    async def test_cancel_order_unauthorized(
        ...
        with pytest.raises(
            ...
            await late_night_service.cancel_order(order_id, user)
    ...
    async def test_update_order_invalid_item_type(
        ...
        order = OrderDTO(
            ...
            total=Decimal("15.59"),  # 12.99 + 20% surcharge
            customer_email=user.email,
            notes="Test order",
        ...
        with pytest.raises(
            ...
            await late_night_service.update_order(order_id, order_input, user)

    @pytest.mark.asyncio
    @freeze_time("2024-03-20 23:00:00")  # 11 PM
    async def test_cancel_order_invalid_service_type(
        self,
        late_night_service: LateNightOrderService,
        mock_order_dao: AsyncMock,
        mock_pizza_dao: AsyncMock,
        ...
        order = OrderDTO(
            ...
            total=Decimal("17.99"),  # 12.99 + 5.00 delivery fee
            customer_email=user.email,
            notes="Test order",
            created_at=datetime.now(),
            updated_at=datetime.now(),
        ...
        # Mock the pizza object with a proper name attribute
        pizza_mock = Mock()
        pizza_mock.name = "Margherita"
        pizza_mock.id = pizza_id
        mock_pizza_dao.get_by_id.return_value = pizza_mock

        # When/Then
        with pytest.raises(
            ValueError, match="Cannot cancel non-late-night order"
        ):
            await late_night_service.cancel_order(order_id, user)
