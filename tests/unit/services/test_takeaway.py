...
class TestTakeawayService:
    @pytest.mark.asyncio
    async def test_create_order_success(
        self,
        takeaway_service: TakeawayOrderService,
        ...
        # When
        result = await takeaway_service.create_order(order_input, user)
    ...
    async def test_create_order_invalid_service_type(
        ...
        # Given
        order_input = OrderInputDTO(
            service_type=ServiceType.DELIVERY,  # Invalid for takeaway service
            items=[
                OrderItemInputDTO(
                    type="pizza",
                    product_name="Margherita",
        ...
        # When/Then
        with pytest.raises(
            ValueError, match="Invalid service type for takeaway service"
        ):
            await takeaway_service.create_order(order_input, user)
    ...
    async def test_check_status_success(
        ...
        # When
        result = await takeaway_service.check_status(order_id, user)
    ...
    async def test_check_status_order_not_found(
        ...
        with pytest.raises(NotFoundError, match=f"Order {order_id} not found"):
            await takeaway_service.check_status(order_id, user)
    ...
    async def test_update_order_success(
        ...
        # When
        result = await takeaway_service.update_order(
    ...
    async def test_update_order_not_found(
        ...
        # When/Then
        with pytest.raises(NotFoundError, match=f"Order {order_id} not found"):
            await takeaway_service.update_order(order_id, update_input, user)
    ...
    async def test_update_order_invalid_service_type(
        ...
        # When/Then
        with pytest.raises(
            ValueError, match="Cannot change service type to non-takeaway"
        ):
            await takeaway_service.update_order(order_id, update_input, user)
    ...
    async def test_cancel_order_success(
        ...
        # When
        result = await takeaway_service.cancel_order(
    ...
    async def test_cancel_order_unauthorized(
        ...
        with pytest.raises(
            ...
            await takeaway_service.cancel_order(order_id, user)
    ...
    async def test_update_order_invalid_item_type(
        ...
        with pytest.raises(
            ...
            await takeaway_service.update_order(order_id, order_input, user)
